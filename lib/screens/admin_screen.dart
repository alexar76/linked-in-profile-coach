...
class AdminScreenState extends State<AdminScreen> {
  ...
  Future<void> _addProfile() async {
    final name = await _promptProfileName(
      ...
      confirm: context.l10n.profilesAddConfirm,
    );
    if (name == null) return;
  ...
  Future<void> _deleteProfile(ManagedProfile profile) async {
    if (_profiles.length <= 1) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(context.l10n.profilesCannotDeleteLast)),
      );
      return;
    }
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(context.l10n.profilesDeleteTitle),
        content: Text(context.l10n.profilesDeleteBody(profile.label)),
        actions: [
          TextButton(
          ...
          FilledButton(
            ...
            child: Text(context.l10n.profilesDeleteConfirm),
          ),
        ],
      ),
    );
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            hintText: context.l10n.profilesNameHint,
          ),
        ),
        actions: [
          TextButton(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                Text(
                  context.l10n.profilesSectionSubtitle,
                  style: TextStyle(color: Colors.grey.shade600, fontSize: 13),
                ),
                const SizedBox(height: 12),
                Text(
                  context.l10n.profilesCount(_profiles.length),
                ...
                ..._profiles.map((p) {
                  ...
                  return ListTile(
                    ...
                    title: Text(p.label),
                    subtitle: p.targetRole.trim().isNotEmpty
                        ? Text(p.targetRole)
                        : Text(p.name),
                    trailing: _profiles.length > 1
                        ? IconButton(
                            icon: const Icon(Icons.delete_outline),
                            tooltip: context.l10n.profilesDeleteAction,
                            onPressed: () => _deleteProfile(p),
                          )
                        : null,
                    onTap: isActive ? null : () => _switchProfile(p.id),
                  );
                }),
                const SizedBox(height: 8),
                OutlinedButton.icon(
                  ...
                  label: Text(context.l10n.profilesAddAction),
                ),
              ],
            ),
          ),
